...
public class ComboCommands{
  ...
  public ParallelCommandGroup startAmpIntakeCommand() {
    return new ParallelCommandGroup(
           ...
           intake.flipToGroundAndRunPayloadCommand(intake.intakeToBeamBreak(), 0, 0)
    );
}
...
public ParallelCommandGroup startShooterIntakeCommand() {
    return new ParallelCommandGroup(
            elevator.goToBottomCommand(),
            intake.flipToGroundAndRunPayloadCommand(intake.startSpin(), 0, 0),
...
public ParallelCommandGroup stopIntakeCommand() {
    return new ParallelCommandGroup(
            intake.flipToStowAndRunPayloadCommand(intake.stopSpin(), 0, 0)
    );
}
...
public ParallelCommandGroup ampCommands() {
    return new ParallelCommandGroup(
            ...
            intake.flipToGroundAndRunPayloadCommand(intake.ejectCommand(), .5, .5)
    );
}
...
public ParallelCommandGroup noteTransferToShooter() {
    return new ParallelCommandGroup(
            elevator.goToBottomCommand(),
            intake.flipToGroundAndRunPayloadCommand(intake.startSpin().until(intake::intakeEmpty), .5, .5),
            transfer.intakeToShooterCommand()
    );
}
...
public SequentialCommandGroup noteTransferToIntake() {
    return new ParallelCommandGroup(
        ...
        intake.ejectCommand().until(intake::outerIntakeFull), .5, .5)
    )
